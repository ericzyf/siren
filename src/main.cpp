  ...
  while (res >= 0) {
    ...
    if (isPlanarAudio) {
      ...
      for (int i = 0; i < codecCtx->channels; ++i) {
        data[i].insert(data[i].end(), frame->data[i],
                       frame->data[i] + lineSize);
      }
    } else {
...
int pa_stream_cb(const void *inputBuffer, void *outputBuffer,
                 ...
                 PaStreamCallbackFlags statusFlags, void *userData)
{
      ...
      if (ctx->packet->stream_index == ctx->audioStreamIndex) {
        ...
        if (ret < 0) {
          ...
          if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF &&
              ret != AVERROR(EINVAL) && ret != AVERROR(ENOMEM)) {
        ...
        const bool isPlanarAudio =
            av_sample_fmt_is_planar(ctx->codecCtx->sample_fmt);
  ...
  auto maxNumFrames =
      std::min(frameCount, (decltype(frameCount))(ctx->frameQueue->size()));
...
bool getPaSampleFormat(const AVCodecParameters *codecParams,
                       PaSampleFormat &fmt)
{
...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    options.add_options()("f,file", "Path to the audio file",
                          cxxopts::value<std::string>());
    options.add_options()("v,verbosity", "Set log verbosity(0~6)",
                          cxxopts::value<int>()->default_value("2"));
    options.add_options()("s,samplerate", "Set sample rate(0: auto)",
                          cxxopts::value<int>()->default_value("0"));
    options.add_options()(
        "b,bufferframes", "Set number of buffer frames",
        cxxopts::value<unsigned long>()->default_value("512"));
    options.add_options()("listhostapi", "List host audio API");
